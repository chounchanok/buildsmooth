...
@section('subhead')
    <title>นัดหมายทั้งหมด - AMC AIR</title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.13.2/themes/base/jquery-ui.min.css" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="{{ asset('dist/css/datatables.min.css') }}">
@endsection

@section('subcontent')
    <h2 class="intro-y text-lg font-medium mt-10">นัดหมายทั้งหมด</h2>
                    ...
                    @foreach ($order as $_order)
                        ...
                        <?php 
                            ...
                            if(!empty($_work_order->work_date)){
                                ...
                                $dayName = 'วัน'.\Carbon\Carbon::parse($workDate)->locale('th')->translatedFormat('l').'ที่ ';
                            }else{
                                $dayName = '';
                            }
                        ?>
                        <tr class="intro-x">
                            ...
                            <td class="text-center">{{ $dayName.(!empty($_work_order->work_date) ? date('d/m/Y', strtotime($_work_order->work_date)).' '.$_work_order->work_timestart.' ถึง '.$_work_order->work_timeend : '-') }}</td>
                            <td class="w-40">
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a href="{{ url('book_form_edit').'/'.$_order->order_id }}" class="flex items-center mr-3 <?php echo ($_order->order_status == 1 ? 'text-success' : 'text-info' ); ?> " href="javascript:;">
                                        <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> {{ ($_order->order_status == 1 ? 'ทำนัดหมาย' : ($_order->order_status == 4 ? 'ตรวจสอบการดำเนินการ' : 'แก้ไขรายการ') ) }}
                                    </a> 
                                </div>
                            </td>
                        </tr>
