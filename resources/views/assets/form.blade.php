...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12">
            <!-- BEGIN: Form Layout -->
            <form action="{{ isset($asset) ? route('assets.update', $asset->asset_id) : route('assets.store') }}" method="POST">
                @csrf
                @if (isset($asset))
                    @method('PUT')
                @endif
                ...
                <div class="intro-y box p-5">
                    ...
                    <div class="grid grid-cols-12 gap-x-5 mt-3">
                        <div class="col-span-12 xl:col-span-4">
                         ...
                         <div class="col-span-12 xl:col-span-4 mt-3 xl:mt-0">
                            <label for="assigned_to_user_id" class="form-label">ผู้ดูแล</label>
                            <select id="assigned_to_user_id" name="assigned_to_user_id" class="form-select w-full">
                                <option value="">-- ไม่กำหนด --</option>
                                 @php
                                    $currentUser = old('assigned_to_user_id', $asset->assigned_to_user_id ?? '');
                                @endphp
                                @foreach ($users as $user)
                                    <option value="{{ $user->user_id }}" {{ $currentUser == $user->user_id ? 'selected' : '' }}>
